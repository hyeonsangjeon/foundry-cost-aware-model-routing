...
*"언제나 더 낮은 비용"*이 아닙니다. **싼 모델이 실제로 통과할 때만** 절감이 생깁니다. 여기서는 싼 모델이 모두
...
!!! success "양방향 재현성 계약 (`max_delta_pct`)"
    ...
    두 번째 상한은 새로 추가한 가드입니다. 미래의 변경이 신호를 약하게 만들거나 비용 계산을
    잘못해 이 워크로드를 "더 싸" 보이게 만들면 `savings_ceiling` 체크가 실패합니다. 모든
    태스크가 여전히 최상위 모델을 필요로 하는데 절감을 보고하지 못하게 막습니다.
