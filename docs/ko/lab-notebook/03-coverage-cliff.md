...
후보 정책의 비용은 $1.66에서 $0.73으로 낮아졌지만 푼 태스크도 줄었습니다. 태스크의 33%는
남은 모델이 모두 검사에 실패했습니다. 비싼 fallback을 빼면서 이 태스크를 통과할 마지막
모델, 곧 "보장된 깨끗한 마지막 후보"도 없어졌습니다.
...
!!! danger "delta_pct를 곧이곧대로 비교하지 마세요"
    ...
    (`$1.19` vs 시드의 `$2.23`). 그래서 "38.9% > 25.5%"는 **더 나은 절감이 아닙니다.**
    더 작은 기준선과 비교하는 동안 통과한 태스크도 줄었습니다. **커버리지를 고정하지 않은 비용
    비교는 무의미합니다.**

그래서 시드 정책은 비싼 fallback을 남깁니다. 대부분의 태스크는 이 모델을 쓰지 않아 비용
영향이 작습니다. 어려운 태스크는 싼 모델이 실패했을 때 이 모델을 써서 커버리지 100%를
유지합니다. 핵심 주장은 *"같은 커버리지,
더 낮은 비용"*이므로 커버리지가 낮아진 결과는
해당하지 않습니다.
...
- fallback 모델을 빼서 비용을 낮추면 푼 태스크도 줄어든다는 점을 설명할 때.
