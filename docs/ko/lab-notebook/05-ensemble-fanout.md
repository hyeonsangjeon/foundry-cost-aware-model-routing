...
<figure markdown="span">
  ...
  <figcaption>앙상블 루프 — 워크로드가 다섯 후보를 병렬 호출하고 가장 싼 통과 후보를 남긴다. 나머지 네 호출에도 비용이 든다.</figcaption>
</figure>
...
- **작업(무엇을):** 손으로 고른 가치 높은 태스크 6건에, 값싼 후보는 검사 하나를 실패하고
  ...
  **compare 모드**로 보내 모든 후보를 평가(팬아웃)하고 동점은 정책 순위상 **가장 싼 완전
  통과 모델**로 깨집니다.
- **실험(무엇을 검증):** (1) 라우팅이 **커버리지 100%**를 유지하고, (2) 나이브보다
  **~47% 절감**하며, (3) 공용 메트릭이 모든 후보 호출 비용과 선택된 승자 비용의 차이를
  기록하는지 확인합니다.

실험 01·02는 절감을, 03은 줄어든 커버리지를, 04는 절감이 없는 워크로드를 보여 줍니다. 이
실험은 앙상블이 추가로 만든 호출을 기록합니다.
기존 요약의 "앙상블은 공짜가 아니다 — 팬아웃에는 세금이 붙는다"는 말이 이 추가 비용을
가리킵니다.

## 승자 비용에 모든 호출이 들어가지 않는 이유

`route_tasks`가 compare 모드로 모든 후보를 평가해도 트레이스의 `cost_usd`에는 **선택된 모델
하나**만 들어갑니다. `total_cost_usd`는 라우팅 청구액이지 모든 앙상블 호출 비용은 아닙니다.
전체 팬아웃 비용은 compare 태스크에서 시도한 모든 후보 비용의 합, 곧
"모든 모델을 다 돌리는 앙상블"의 비용입니다.
...
- `ensemble_tax_usd` = `fanout_usd − winner_usd` — 선택되지 않은 후보의 비용
...
| task | class | 팬아웃 후보 | 승자 | 팬아웃 원가 | 승자 비용 | 추가 비용 |
| --- | --- | --- | --- | --- | --- | --- |
...
## 모든 모델을 부르는 방식이 가장 비싸다

합성 100건 히어로 워크로드에도 모든 태스크에서 모든 모델을 부르는 `all_ensemble` 전략을
추가했습니다:
...
`all-ensemble`은 커버리지 100%지만 **가장 비쌉니다**. premium보다도 1.9배입니다. premium도
이미 100%이므로 추가 호출은 이 워크로드의 커버리지를 높이지 않습니다.
...
- `ExperimentMetrics` — 실행 하나의 정규화 스냅샷(비용·커버리지·팬아웃 추가 비용 + 내용 주소
...
앙상블/팬아웃은 **품질을 올릴 수** 있지만 이 실험은 그 효과를 측정하지 않습니다. 여기서는
오프라인 비용만 계산합니다. 같은 커버리지에서 모든 후보를 부르면 승자만 청구할 때보다
비쌉니다. 추가 $0.36과 3.74× 비율만큼 품질이 좋아지는지는 라이브 품질 실험으로 판단해야
합니다.

## 이 실험을 언제 쓰나

- "여러 모델 앙상블/베스트-오브-N을 도입하면 얼마나 더 드나?"에 답할 때.
- 라우팅의 절감을 보고할 때 승자 비용과 모든 후보 호출 비용을 함께 노출할 때.
