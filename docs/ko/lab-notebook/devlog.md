...
!!! note "한 줄 요약"
    실험 09의 measured 런을 canonical 해시 체인 원장으로 굳혀 자격 증명·네트워크 없이도
    누구나 한 줄로 `PASS`를 재현하게 만들었습니다 — **1바이트만 고쳐도 검증이 깨집니다.**

- **상황(왜):** 실험 09는 저장소 최초 `measured = true`였지만 측정 기록은 무결성·재생 없는
...
- **상황(왜):** 측정 원장은 무결성/재생 없는 평면 append-only JSONL이었습니다. 실측 기록이
...
- **검증(효과):** 녹화 스냅샷 금액 = **`$0.020334`**(5-시리즈 리스트 요율 — Model Router
  input 마크업이 빠져 **불완전**, 히스토리 출력으로만 유지), 커버리지 **미채점**
  (usage는 측정, 정확도는 grader 필요). 각 항목 `provenance = recorded`라 **재생이 절대
...
- **검증(효과):** 표의 모든 수치를 실측으로 재확인 — 01 −25.5% · 02 −56.7% · 03
  ...
  없음(순수 문서 종합). 모든 수치 `measured = false`. 이 대표 수치의 정본은
  [오프라인 실험 결과](../manual/projection-results.md)입니다.
