...
> **태스크를 통과할 수 있는 가장 싼 모델부터 시도합니다. 결과를 검사하고 실패하면 다음 모델을
> 부릅니다. 통과율 상승이 추가 비용만큼 가치가 있을 때만 더 비싼 모델을 쓰고, 결과를 검증할
> 증거를 기록합니다.**

이 페이지는 프로젝트 설치, 실험 실행, 결과 확인, 재현 방법을 정리합니다. 실험은 두 종류입니다.
하나는 실제 Azure Foundry를 호출해 측정한 실측 결과이고(실험 09·10·11·12), 다른 하나는 합성
데이터로 라우팅 로직을 검증하는 오프라인 실험입니다(실험 01–08). 오프라인 실험은 네트워크나
외부 호출 없이 같은 입력이면 같은 결과가 나옵니다.
...
!!! abstract "내장 Model Router가 모델을 고른 뒤 이 저장소가 하는 네 가지"
