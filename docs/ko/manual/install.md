...
이 빠른 시작이 실행하는 실험(투영 트랙 01–08)은 **오프라인·결정론적**입니다. 네트워크도, 자격
증명도, 외부 API 호출도 없습니다. 저장소에 포함된 합성 샘플만으로 모든 것이 동일하게
재현됩니다. 실측 트랙(실험 09·10·11·12)은 실제 Azure Foundry 호출이라 테넌트 설정이 필요합니다.

## 요구사항

- **Python 3.11 또는 3.12** — CI가 설치하고 실행하는 두 인터프리터입니다. 더 새로운
  버전은 주장하지 않으므로 `pip`이 설치 단계에서 거절합니다.
