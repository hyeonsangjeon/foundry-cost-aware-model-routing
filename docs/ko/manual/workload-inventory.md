...
**투영 트랙(실험 01–08)은 여전히 프로젝션입니다** — 위 100태스크 텔레메트리에는 프롬프트가
없어 오프라인 신호로 라우팅을 *투영*만 하기 때문입니다(`measured = false`). 하지만
**실측 트랙(실험 09·10·11·12)은 이미 `measured = true`로 측정됐습니다.** 실험 09·10은 위
`curated-arena-live`(5 태스크)로 실제 Foundry 라우팅을 캡처·봉인했고 프롬프트+기계 검증을
갖춘 `curated-24`(24 태스크)로는 실험 11·12가 유료 4-arm 실측을 돌렸습니다 — 실험 11이 $3.47,
실험 12가 $3.27(예산 각 $20)을 실제로 지출했습니다. 아래는 실측 워크로드의 현재 상태입니다:

| 실측 워크로드 | 규모 | `evidence_tier` | 겨냥 실험 | 상태 |
| --- | --- | --- | --- | --- |
| `curated-24` | 중간(24) | **`directional`** | 11 · 12 (03D) | ✅ **실측 완료** (11 $3.47 · 12 $3.27, 11은 사전등록 미달 VOID) |
...
!!! note "정직성 경계"
    이 표는 **구현된 현재 상태**입니다. `curated-24`는 승인·확정돼 실험 11·12가 유료 실측
    (`measured = true`)으로 돌았고(실험 11은 사전등록 기준 미달로 **무효(VOID)** 판정 — 무효여도
    측정은 측정입니다), 실험 09·10도 라이브 라우팅 캡처로 `measured = true`입니다. 반면
    `hero-100-prompts`는 아직 초안·승인 대기라, 투영 트랙(실험 01–08)의 수치는 여전히
    `measured = false` 프로젝션입니다. 실측 워크로드의 태스크·프롬프트는 콘텐츠 설계라 **초안을
    올리고 운영자 승인 후 확정**합니다 — 결과를 보고 태스크를 고치지 않습니다(exp04의 교훈).
