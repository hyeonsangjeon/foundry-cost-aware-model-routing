...
!!! info "정직 라벨"
    ...
    - `coverage 96.18%` (277/288) — content-graded 셀 비율. arm 최저 94.4% (모두 게이트 90% 통과).
...
타임아웃 셀은 **이중 보수 처리**된다: (1) content가 없으니 coverage에서 **제외**되고, 동시에
...
- **품질 게이트 판정**(prereg 고정 기준): coverage ≥ 90% **PASS** · min_pass ≥ 0.60 **PASS** ·
