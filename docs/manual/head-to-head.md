...
!!! info "네 방법 = 내장 라우터 위에 얹히는 축들의 대비"
    ...
    (배포 하나·크로스 프로바이더). 이 네 방법 비교는 그 **위 층**을 보여줍니다 — **앙상블(팬아웃)**
    은 [앙상블 세금 차별점](../lab-notebook/05-ensemble-fanout.md)(승자만 vs 전부 합산)을,
    **비용 인지 라우터** 는 *관찰-후-에스컬레이션*(검증 기반 채택 + 비용 거버너)을 대표합니다. 즉
    내장 라우터의 단일 선택과, 이 저장소가 더하는 **검증·에스컬레이션·거버넌스**의 대비를 한
    태스크로 좁혀 본 것입니다 — 그 집계판이 [실험 07 · 라우팅 레이어](../lab-notebook/07-model-router.md) ⭐.
