# 실험 12 · 라우팅 모드 유료 실측 (재런) — **사전등록 규율 아래 나온 publishable 결과**
...
!!! warning "이 페이지도 실제 유료 런을 기록한다 — operator가 승인한 지출"
    ...
    입니다. 총지출 **$3.269553 / 예산 $20.00**, 키리스 Entra, 순차 실행, 카운터밸런스 arm 순서,
    고정 시드. 프롬프트·응답 **원문은 공개되지 않습니다** — 봉인 스냅샷은 로컬(gitignored)에
    남고, 공개 흔적에는 `output_sha256`(채점 증거)만 실립니다.

## 무엇을 고쳤나 — 음성 결과가 지목한 두 원인만

실험 11은 세 가지를 발견했고, 그중 **런을 무효로 만든 두 원인**을 이 재런에서 고쳤습니다.
게이트·estimand·워크로드·시드는 **하나도 바꾸지 않았습니다.**
