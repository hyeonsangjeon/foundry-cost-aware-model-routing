# 실험 11 · 라우팅 모드 유료 실측 — **prereg 규율이 무효로 판정한 정직한 음성 결과**
...
!!! abstract "한 줄 요약"
    ...
    내가 결과를 보기 **전에** 커밋해 둔 사전등록(preregistration)이 스스로 정한 커버리지 게이트를
...
- **집계 커버리지는 90.6%(261/288)로 아슬하게 90%를 넘지만**, 게이트는 *arm별*입니다 —
...
quality 커버리지를 79.2%로 끌어내렸습니다 — **런을 무효로 만든 직접 원인**입니다. (반대로 Grok은
...
- **사전등록이 자기 자신을 무효화했다.** 내가 커밋한 게이트(어느 arm이든 커버리지<90% → 무효)가,
...
!!! danger "이 런으로 주장하지 않는 것"
    - **절감률**: `router-quality` 커버리지가 게이트를 못 넘어 **비교 자체가 무효**입니다.
...
| 고칠 것 | 왜 | 효과 |
| --- | --- | --- |
| **`max_output_tokens` 상향** (2048 → 제안 8192) | reasoning 모델이 예산을 추론에 다 써 코드 미출력 | quality 커버리지를 90% 위로 복구 → 비교 유효화 |
