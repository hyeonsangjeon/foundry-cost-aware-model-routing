...
DEMO_STRINGS = {
    ...
    'k112': {
        'en': 'full coverage at less than premium cost',
        'ko': '프리미엄보다 낮은 비용으로 완전한 커버리지',
    },
    'k113': {
        'en': 'Cost &times; coverage &mdash; what each option costs and solves',
        'ko': '비용 &times; 커버리지 &mdash; 각 방식이 얼마를 쓰고 얼마나 푸는지',
    },
    'k114': {
        'en': 'run a replay&#8230;',
        'ko': '리플레이 실행&#8230;',
    ...
    'k118': {
        'en': 'per prompt and stops. With no escalation after a failed result, coverage stays low. The observe-then-escalate',
        'ko': ' 고르고 멈춥니다. 실패한 뒤 상위 모델로 옮기지 않아 커버리지가 낮습니다. 결과를 확인한 뒤 실패할 때만 올리는 ',
    },
    'k119': {
        'en': 'cost-aware mix',
        'ko': '비용 인지 믹스',
    ...
    'k120': {
        'en': 'reaches full coverage at a comparable cost.',
        'ko': '가 비슷한 비용으로 완전한 커버리지에 도달합니다.\n        ',
    },
    'k121': {
    ...
    'k123': {
        'en': 'Run a replay to compare all-mini vs all-premium vs the cost-aware mix. All-mini costs less but solves fewer tasks. All-premium solves them but costs more. The mix keeps full coverage below premium cost.',
        'ko': '리플레이로 all-mini vs all-premium vs 비용 인지 믹스를 비교합니다. all-mini는 비용이 낮지만 푸는 과제가 적고, all-premium은 과제를 풀지만 비용이 높습니다. 믹스는 프리미엄보다 낮은 비용으로 완전한 커버리지를 지킵니다.',
    },
    'k124': {
        'en': 'experiments',
        'ko': '실험',
    ...
    'k127': {
        'en': 'extra cost from calling every candidate',
        'ko': '모든 후보를 부를 때 드는 추가 비용',
    },
    'k128': {
    ...
    'k142': {
        'en': 'Removing expensive fallback models drops coverage',
        'ko': '비싼 폴백 모델을 빼면 커버리지가 낮아진다',
    },
    'k143': {
    ...
    'k159': {
        'en': 'Experiment 03 &middot; coverage after removing fallback models',
        'ko': '실험 03 &middot; 폴백 모델을 뺀 뒤의 커버리지',
    },
    'k160': {
        'en': 'Fan-out setting &mdash; how often to call every candidate',
        'ko': '팬아웃 설정 &mdash; 모든 후보를 얼마나 자주 부를지',
    },
    'k161': {
    ...
    'k163': {
        'en': '. Raise it and the router calls every candidate on fewer tasks. Coverage and savings stay unchanged while the extra calls cost less. Experiment 05 (fan out all) vs 06 (fan out none).',
        'ko': '. 이 값을 올리면 모든 후보를 부르는 과제가 줄어듭니다. 커버리지와 절감은 그대로이고 추가 호출 비용만 낮아집니다. 실험 05(전부 팬아웃) vs 06(전혀 팬아웃 안 함).',
    },
    'k164': {
        'en': '&#9646; extra all-candidate cost',
        'ko': '&#9646; 모든 후보 호출의 추가 비용',
    },
    'k165': {
        'en': ' (falls)',
...
MEASURED = {
    'en': {
        ...
        'armKey': 'Arm labels: <span class="mono">router-cost</span> (Model Router in Cost mode) · <span class="mono">router-balanced</span> (Model Router in Balanced mode) · <span class="mono">router-quality</span> (Model Router in Quality mode) · <span class="mono">direct-premium</span> (calling the premium model directly · <span class="mono">gpt-5.6-sol</span>).',
        'sub': 'The same 24 coding tasks ran through four arms (three routing modes + the direct-premium baseline) at n=3 on real Azure AI Foundry. The result contains 288 cells and replayed byte-identically from a sealed snapshot. This read-only view uses the masked published.json bundle: aggregates only, no prompts, endpoints, or tenant ids.',
        ...
        'armsNote': 'The deployment appears under each arm. Every arm is cost_complete=true (unpriced 0%), and every cell uses pinned rates. Pass rate counts tasks solved/planned. Grading coverage counts cells graded/planned. The denominators are different.',
        'cCost': 'cost',
        'cPass': 'pass rate',
        'cPerPass': '$/pass',
        'cCov': 'coverage',
        'cells': 'cells',
        'head': '<b>router-cost</b> keeps a <b>{qCost}</b> task pass rate while costing <b>{cheaper}% less</b> than direct-premium. The pass-rate gap is within <b>{gap}%p</b>. The timeout section below shows that the gap comes from timeouts, not code quality.',
        'domH': 'Quality mode cost more and passed less than direct-premium',
        'domP': "<b>router-quality</b> ({qCostUsd}) costs more than <b>direct-premium</b> ({premUsd}) and has a lower pass rate ({qQual} &lt; {premQual}). Quality mode reaches expensive models and adds the router markup; calling direct-premium directly avoids that markup. On this workload, the direct call is cheaper and more accurate. Meanwhile router-cost keeps the same {costQual} pass rate at under 1/20 the cost.",
        ...
        'backNote': 'This distribution includes graded cells only; timeout cells whose backend never settled are excluded. Cost mode sent 100% to grok-4-1-fast-reasoning in both measured runs (the prior void run and this publishable one). Quality mode sent no cells to Grok at all.',
        'toTitle': '11 timeout cells — shown, not hidden',
        'toNote': "All 11 are HTTP 408 read timeouts in the router arms; direct-premium had 0. Each timeout is excluded from grading coverage and counted as a failure. Those timeouts account for the entire pass-rate difference between the router arms and direct-premium. The 4.17%p gap comes from latency (router backends p50 ~12–16s vs direct-premium ~4.2s), not code quality.",
        'toByArm': 'By arm',
        'toByTask': 'By task',
        'toArm': 'arm',
        'toTask': 'task',
    ...
    'ko': {
        ...
        'armKey': '실험 arm 라벨: <span class="mono">router-cost</span>(Model Router의 Cost 모드) · <span class="mono">router-balanced</span>(Model Router의 Balanced 모드) · <span class="mono">router-quality</span>(Model Router의 Quality 모드) · <span class="mono">direct-premium</span>(프리미엄 모델 직접 호출 · <span class="mono">gpt-5.6-sol</span>).',
        'sub': '같은 24개 코딩 과제를 실제 Azure AI Foundry에서 네 arm(라우팅 3모드 + direct-premium 기준선)으로 n=3씩 실행했다. 결과는 288셀이며 봉인 스냅샷 replay에서 바이트 단위로 같았다. 이 읽기 전용 화면은 마스킹된 published.json 번들의 집계만 사용하며 프롬프트·엔드포인트·테넌트 식별자는 담지 않는다.',
        'lblCoverage': 'coverage',
        'lblUnpriced': 'unpriced',
        'lblReplay': 'replay verified',
        'lblSpend': 'spend',
        'armsTitle': '네 arm — 비용 · 통과율 · $/pass · 채점 커버리지',
        'armsNote': 'arm 이름 아래에 배포명을 적었다. 모든 arm이 cost_complete=true(unpriced 0%)이고 모든 셀에 고정 요율을 적용했다. 통과율은 해결/계획 과제 수를, 채점 커버리지는 채점/계획 셀 수를 센다. 분모가 다르다.',
        'cCost': '비용',
        'cPass': '통과율',
        'cPerPass': '$/pass',
        'cCov': '커버리지',
        'cells': '셀',
        'head': '<b>router-cost</b>는 통과율 <b>{qCost}</b>를 유지하면서 direct-premium보다 비용이 <b>{cheaper}% 낮다</b>. 통과율 차이는 <b>{gap}%p</b> 이내다. 아래 타임아웃 절에 따르면 이 차이는 코드 품질이 아니라 타임아웃에서 나왔다.',
        'domH': 'Quality 모드는 direct-premium보다 비용이 높고 통과율이 낮았다',
        'domP': '<b>router-quality</b>({qCostUsd})는 <b>direct-premium</b>({premUsd})보다 비용이 높고 통과율은 낮다({qQual} &lt; {premQual}). Quality 모드는 비싼 모델에 도달하면서 라우터 마크업이 붙지만 direct-premium을 직접 부르면 그 마크업이 없다. 이 워크로드에서는 직접 호출이 더 싸고 정확하다. 한편 router-cost는 같은 {costQual} 통과율을 1/20 미만 비용으로 유지한다.',
        'setupTitle': '실험 구성 — 무엇을 돌렸나',
        'setupNote': '봉인 런의 실제 배포·백엔드 식별자다. 아래 이름은 모두 마스킹된 번들에서 그대로 가져왔다. 합성 데이터에 실제 모델명을 붙이면 모델별 성능 주장처럼 보이므로 offline 탭은 합성 placeholder 모델명을 유지한다.',
        'setArms': 'Arm 구성',
...
EXPERIMENT_I18N = {
    'adaptive': {
        'title': {
            'en': 'Turn unnecessary fan-out down to zero and keep the savings',
            'ko': '필요 없는 팬아웃을 0으로 줄이고 절감은 유지하기',
        },
        'summary': {
            'en': "Reuses experiment 05's workload unchanged but turns fan-out off by raising the budget gate's compare_min_value above every task's value. Coverage stays at 100% and savings hold at ~47% versus naive, while the extra fan-out cost falls to ~$0.00. On this deterministic offline projection, fan-out picks the same cheapest-passing model that ordered escalation already finds. Turning it off removes the extra calls with no loss.",
            'ko': '실험 05와 같은 워크로드를 그대로 쓰되 예산 게이트의 compare_min_value를 모든 태스크 가치보다 높여 팬아웃을 끕니다. 커버리지 100%와 나이브 대비 ~47% 절감은 그대로이고 팬아웃 추가 비용은 ~$0.00으로 낮아집니다. 이 결정론적 오프라인 투영에서 팬아웃은 ordered 에스컬레이션이 이미 찾는 가장 싼 통과 모델과 같은 승자를 고릅니다. 팬아웃을 끄면 결과 손실 없이 추가 호출만 없어집니다.',
        },
    },
    'curated': {
        'title': {
    ...
    'ensemble': {
        'title': {
            'en': 'Ensemble fan-out — call every candidate and pay for every call',
            'ko': '앙상블 팬아웃 — 모든 후보를 부르고 모든 호출 비용 내기',
        },
        'summary': {
